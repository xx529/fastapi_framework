...
from app.schema.enum import RedisKeyEnum, RedisActionEnum
...
class UserInfoRepo(BaseRepo):
    ...
    @redis_cache(
        key=lambda page, limit, order_type: f'{RedisKeyEnum.USER_REPO.value}:{page}-{limit}-{order_type}-list',
    ...
    @redis_cache(key=lambda user_id: f'{RedisKeyEnum.USER_REPO.value}:{user_id}-detail')
    def detail(self, user_id: int):
        ...
        data = self.execute(stmt, output=PullDataFormatEnum.RECORDS)
        if len(data) == 0:
            raise AppException.UserNotExist(detail=f'user_id {user_id} not exist')
        else:
            return data[0]

    @redis_cache(key=lambda user_id: f'{RedisKeyEnum.USER_REPO.value}:{user_id}*', action=RedisActionEnum.CLEAR)
    @redis_cache(key=lambda: f'{RedisKeyEnum.USER_REPO.value}:*-list', action=RedisActionEnum.CLEAR)
