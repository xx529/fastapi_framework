...
from .logger import middleware_log, request_log
...
class MiddleWare:
    ...
    async def log_request(request: Request, call_next):
        ...
        request_log.info(f'{request.method} {request.url}')
        middleware_log.debug(f'headers: {dict(request.headers)}')
        response = await call_next(request)

        if response.status_code in RequestSuccessCodeEnum.list():
            request_log.info(f'status code: {response.status_code}')
        else:
            request_log.error(f'status code: {response.status_code}')
